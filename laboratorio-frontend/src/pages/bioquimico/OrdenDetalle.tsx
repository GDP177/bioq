// src/pages/bioquimico/BioquimicoOrdenDetalle.tsx
...
export default function BioquimicoOrdenDetalle() {
  ...
  // El param depende de cómo lo llamaste en tus rutas, comúnmente 'id' o 'id_orden'
  const { id, id_orden } = useParams(); 
  const orderId = id || id_orden;
  ...
  const handlePrint = useReactToPrint({ contentRef: componentRef, documentTitle: `Orden_${orderId}` });

  useEffect(() => {
    if (orderId) cargarOrdenDetalle(parseInt(orderId));
  }, [orderId]);

  const cargarOrdenDetalle = async (ordenIdToFetch: number) => {
    try {
      setLoading(true);
      // Consumimos el backend (usamos api/ordenes ya que está unificada en el backend)
      const response = await axios.get(`http://localhost:5000/api/ordenes/${ordenIdToFetch}`);
      
      if (response.data.success) {
        const raw = response.data.orden;
        const analisisList = response.data.analisis || [];

        // Mapeamos los datos al formato que espera esta interfaz (igual que en el médico)
        const ordenMapeada: OrdenDetalle = {
          id: raw.id_orden,
          nro_orden: raw.nro_orden,
          fecha_ingreso: raw.fecha_ingreso || raw.fecha_ingreso_orden,
          estado: raw.estado,
          urgente: raw.urgente === 1,
          observaciones_medico: raw.observaciones,
          costo_total: raw.costo_total || 0,
          matricula_bioquimico: raw.matricula_bioquimico || raw.matricula_bq || "Pendiente",
          ...
          resumen: {
            ...
            analisis_finalizados: analisisList.filter((a:any) => a.estado === 'finalizado').length,
            porcentaje_completado: Math.round((analisisList.filter((a:any) => a.estado === 'finalizado').length / (analisisList.length || 1)) * 100)
          }
        };
      ...
      <header className="bg-white shadow-sm border-b sticky top-0 z-10 print:hidden">
        <div className="max-w-7xl mx-auto px-4 py-4 flex justify-between items-center">
          <div className="flex items-center gap-4">
            {/* Vuelve a la bandeja del bioquímico */}
          ...
          <div className="flex gap-3">
            {/* BOTÓN EXTRA: Permite al bioquímico ir a cargar resultados */}
            <Button 
                variant="ghost" 
                onClick={() => navigate(`/bioquimico/orden/${orden.id}/cargar`)} 
                className="flex items-center gap-2 shadow-sm text-sm border-blue-200 text-blue-700 hover:bg-blue-50"
            >
              ✏️ Editar Resultados
            </Button>
