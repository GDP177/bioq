...
export default function GestionPacientes() {
  ...
  const cargarPacientes = async (medicoId: number) => {
    try {
      ...
      setError("Error al cargar los pacientes. Verifique la conexión.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const verDetallesPaciente = async (paciente: Paciente) => {
    // 1. Mostrar modal inmediatamente con los datos que ya tenemos
    setPacienteSeleccionado(paciente);
    setMostrarDetalles(true);
    setLoadingDetalle(true);

    try {
      // 2. Pedir datos completos al backend
      const response = await axios.get(
        `http://localhost:5000/api/paciente/ficha/${paciente.nro_ficha}`
      );
      
      if (response.data.success) {
        // 3. Actualizar con la info completa (merge seguro)
        setPacienteSeleccionado(prev => prev ? ({ ...prev, ...response.data.paciente }) : response.data.paciente);
      }
    } catch (error) {
  ...
  // 🛡️ HELPER CRÍTICO: Obtiene el nombre sin importar cómo lo mande el backend
  const getNombreSeguro = (p: Paciente | null) => {
    if (!p) return { nombre: "Desconocido", apellido: "", completo: "Desconocido", iniciales: "??" };
    
    // Intenta leer 'nombre' (Frontend) O 'Nombre_paciente' (Backend)
    const nombre = p.nombre || (p as any).Nombre_paciente || "";
    const apellido = p.apellido || (p as any).Apellido_paciente || "";
    
    const completo = `${nombre} ${apellido}`.trim() || "Sin Nombre";
    const iniciales = `${nombre[0] || ""}${apellido[0] || ""}`.toUpperCase() || "?";

    return { nombre, apellido, completo, iniciales };
  };
                ...
                <table className="min-w-full divide-y divide-gray-200">
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {pacientes.map((paciente) => {
                      // 🛡️ USAMOS EL HELPER SEGURO AQUÍ
                      const datos = getNombreSeguro(paciente);
                        ...
                        <tr key={paciente.nro_ficha} className="hover:bg-blue-50/50 transition-colors">
                          <td className="px-6 py-4 whitespace-nowrap">
                            <div className="flex items-center">
                              ...
                              <div>
                                  <div className="text-sm font-bold text-gray-900">{datos.completo}</div>
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                            <div className="flex justify-end items-center gap-2">
                                ...
                                <button
                                  ...
                                  className="bg-white text-blue-600 border border-blue-200 hover:bg-blue-50 hover:border-blue-300 font-medium px-3 py-1.5 rounded-md text-xs transition-all shadow-sm"
                                >
                                  Ver Ficha
                                </button>
                                
                                {/* BOTÓN: NUEVA ORDEN (VERDE) */}
                                <button
                                  onClick={() => navegarANuevaOrden(paciente)}
                                  className="bg-green-600 text-white hover:bg-green-700 font-medium px-3 py-1.5 rounded-md text-xs transition-all shadow-sm"
                                >
                                  Nueva Orden
                                </button>

                                {/* BOTÓN: HISTORIAL (VIOLETA) */}
                                <button
                                  onClick={() => verHistorialCompleto(paciente)}
                                  className="text-gray-500 hover:text-purple-600 hover:bg-purple-50 p-2 rounded-md transition-all"
            ...
            <div className="bg-blue-600 p-6 text-white flex justify-between items-start sticky top-0 z-10 shadow-md">
              <div className="flex items-center gap-4">
                <div className="w-16 h-16 bg-white/20 rounded-full flex items-center justify-center text-2xl font-bold backdrop-blur-md border-2 border-white/30">
                    {getNombreSeguro(pacienteSeleccionado).iniciales}
                </div>
                <div>
                  <h2 className="text-2xl font-bold">
                    {loadingDetalle ? "Cargando..." : getNombreSeguro(pacienteSeleccionado).completo}
                  </h2>
              ...
              {loadingDetalle ? (
                ...
                <>
                  ...
                  <div className="flex justify-end gap-3 pt-6 border-t border-gray-200 mt-4">
                      <Button variant="ghost" onClick={cerrarModal} className="text-gray-500 border border-transparent hover:border-gray-300">
                          Cerrar
                      </Button>
                      <Button 
...
// Pequeño componente Button interno para evitar errores si no lo tienes importado
function Button({ children, className, onClick, variant = 'primary', ...props }: any) {
    ...
    if (variant === 'ghost') variantStyle = "bg-transparent hover:bg-gray-100 text-gray-700";
    else if (className) variantStyle = className; // Si pasas className, usa eso
    
    return (
        <button className={`${baseStyle} ${variantStyle}`} onClick={onClick} {...props}>
            {children}
        </button>
    );
}